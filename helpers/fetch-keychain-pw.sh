...
if ! keychain_pw=$(imp-vault read-key --log_level=debug --environment=production --vault_role="continuous-integration-production-improbable-iam" --key="${keychain_pw_path}" --field=token); then
